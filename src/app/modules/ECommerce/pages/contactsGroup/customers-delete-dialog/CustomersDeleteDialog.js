          ...
          setSnackbar({
            ...
            message: (
              <p style={{ fontSize: '16px' }}>
                {!error ? `Contacts group deleted successfully!` : error}
              </p>
            ),
            show: true,
          })
    ...
    >
      ...
      <Modal.Header closeButton>
        <Modal.Title id="example-modal-sizes-title-lg">
          Contacts group Delete
        </Modal.Title>
      </Modal.Header>
      <Modal.Body>
        {!isLoading && (
          <span>Are you sure to permanently delete selected contacts group?</span>
        )}
        {isLoading && <span>Contacts group are deleting...</span>}
      </Modal.Body>
      <Modal.Footer>
        <div>
          <button
